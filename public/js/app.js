...
function pageTarifsActions() {
  var rez = tarCalculate('datepicker-from', 'datepicker-to');
  console.log('formula calculated rez=' + rez);
  rez = rez.toFixed(2) * 1;
  var eur = Math.floor(rez);
  var cents = rez - eur;
...
function tarCalculate(from_id, to_id) {
  var tarif_class = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'tarifs-in-days';
  var from = new Date(document.getElementById(from_id).value);
  var to = new Date(document.getElementById(to_id).value);

  if (!(from.getTime() > 0 & to.getTime() > 0)) {
    return false;
  }

  var secDiff = to.getTime() - from.getTime();
  if (secDiff < 0) return 0;
  var dayDiff = secDiff / (1000 * 60 * 60 * 24); // divide by number of miliseconds in day

  var htmlTars = document.querySelectorAll('.' + tarif_class);
  var tars = new Map();
  var tarsKeys = [];
  htmlTars.forEach(function (element) {
    tars.set(Number(element.name), Number(element.value));
    tarsKeys.push(Number(element.name));
  });
  tarsKeys.sort(function (a, b) {
    return b * 1 - a * 1;
  });
  var key = 0;
  var keyPrev = 0;
  ...
  for (var _i = 0, _tarsKeys = tarsKeys; _i < _tarsKeys.length; _i++) {
    var _e = _tarsKeys[_i];
    i++;

    if (dayDiff >= _e && i === 1) {
      key = _e; //console.log('first condition. key:'+key+'_keyPrev='+keyPrev+'_e='+e);

      break;
    } else if (i !== 1) {
      if (dayDiff <= keyPrev && dayDiff > _e) {
        key = keyPrev; //console.log('second condition. key:'+key+'_keyPrev='+keyPrev+'_e='+e);
    ...
    keyPrev = _e; //console.log('no condition key:'+key+'___'+'keyprev:'+keyPrev);
  }
...
document.querySelectorAll('[data-targetinput]').forEach(function (el) {
  el.addEventListener('keyup', krInputAjust);
});
backCallForm = document.querySelector('.back-coll');
backCallForm.querySelector('.close-cross').addEventListener('click', function (e) {
...
function krInputAjust(event) {
  ...
  var text = event.target.placeholder;
  var parent = event.target.parentElement; //console.log(text);
...
var back = document.querySelector('.backgound');
...
var firstDelayFinished = false;

function delayShow(e) {
  menuEvent = e; //console.log(delayStarted,firstDelayFinished);
...
showMenuItem() {
  ...
  var currentA = e.target.querySelector('.item-a');
  e.target.classList.add('hover');
  back.classList.add('show');
  menuItems.forEach(function (el2) {
...
var topMenu1 = document.querySelector('.header-line1 ul');
...
var katalogRazdelUl = document.querySelector('ul[data-navlevel="razdel"]');
var wholeKatalogsDiv = document.querySelector('.mobile-topmenu-container');
var allSubrazdelsUls = wholeKatalogsDiv.querySelectorAll('ul[data-navlevel="subrazdel"]');
...
function hideAllSubRazdels() {
  allSubrazdelsUls.forEach(function (el) {
    el.classList.remove('left');
    el.classList.add('right');
  });
}

function getSubRazdelShown() {
  // shown or false
  allSubrazdelsUls.forEach(function (el) {
...
function toggleKatalog(e) {
  if (katalogRazdelUl.classList.contains('left')) {
    //if razdel hidden
    if (sr = getSubRazdelShown()) {
      //subrazel is shown
      hideKatalogAll();
    } else {
...
document.querySelectorAll('.nav-item-mobile.expand').forEach(function (el) {
...
var itemsFList = document.querySelector('.top-card-container');
document.querySelectorAll('.top-card-arrow').forEach(function (el) {
  el.addEventListener('click', itemsScroll);
});
var scrollContainer = document.querySelector('.top-card-container');
var firstItem = document.querySelector('.top-card-container .top-card');
var cardsNumber = document.querySelectorAll('.top-card-container .top-card').length; //window.addEventListener('resize', centreSliderScroll);

function centreSliderScroll() {
  var itemWidth = firstItem.clientWidth;
  var screenWidth = scrollContainer.clientWidth;
  var scrollToNumber = Math.ceil(cardsNumber / 2); //console.log(scrollToNumber*itemWidth);
...
function itemsScroll(e) {
  var showCapacity = Math.floor(scrollContainer.clientWidth / firstItem.clientWidth);
  var itemsHiddenFullLeft = Math.floor(scrollContainer.scrollLeft / firstItem.clientWidth);
  var itemsHiddenPartLeft = Math.ceil(scrollContainer.scrollLeft / firstItem.clientWidth);
  var itemsShownFullLeft = Math.floor((scrollContainer.scrollLeft + scrollContainer.clientWidth) / firstItem.clientWidth);
  var itemWidth = firstItem.clientWidth;
  var screenWidth = scrollContainer.clientWidth; //console.log(itemsHiddenFullLeft, itemsShownFullLeft, showCapacity, itemWidth,scrollContainer.clientWidth);
...
var reviewTransitionRunning = false;
var REVIEW_CONTAINER = document.querySelector('.review-slider-container');

if (REVIEW_CONTAINER) {
  REVIEW_CONTAINER.addEventListener('transitionend', transitionFinished);
}

var REVIEW_ITEMS = REVIEW_CONTAINER === null || REVIEW_CONTAINER === void 0 ? void 0 : REVIEW_CONTAINER.querySelectorAll('.review-item');

function transitionFinished() {
  var currentItem = REVIEW_CONTAINER.querySelector('.current');
  var nextItem = REVIEW_CONTAINER.querySelector('.next');
...
function showNextReview() {
  var nextForse = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : -1;
  if (reviewTransitionRunning == true) return;
  reviewTransitionRunning = true;
  var currentItem = REVIEW_CONTAINER.querySelector('.current');
  var currentIndex = Array.prototype.indexOf.call(REVIEW_CONTAINER.children, currentItem);
  var currentItemRight = currentItem.parentElement.offsetWidth - currentItem.offsetWidth - currentItem.offsetLeft;
  var nextIndex;
  var nextItem;
  ...
  nextItem.classList.add('next'); //console.log(currentIndex, nextIndex);
  ...
  setPageActive(nextIndex); // console.log(currentItemRight);
}
...
function showPreviousReview(nextForse) {
  ...
  var currentItem = REVIEW_CONTAINER.querySelector('.current');
  var currentIndex = Array.prototype.indexOf.call(REVIEW_CONTAINER.children, currentItem);
  var currentItemLeft = currentItem.offsetLeft;
  var nextIndex;
  var nextItem;
...
var PAGES_CONTAINER = document.querySelector('.pages');
...
function pageClick(e) {
  ...
  var active = PAGES_CONTAINER.querySelector('.active');
  var currentIndex = Array.prototype.indexOf.call(PAGES_CONTAINER.children, e.target);
  var activeIndex = Array.prototype.indexOf.call(PAGES_CONTAINER.children, active);
...
function swipeHandle() {
  var touchDiv = document.querySelector('.review-slider-container');
  if (!touchDiv) return;
  ...
  touchDiv.addEventListener('touchstart', function (e) {
    var touchobj = e.changedTouches[0]; //console.log(touchobj);
  ...
  touchDiv.addEventListener('touchmove', function (e) {
    ...
    if (Math.abs(distX) > Math.abs(distY)) e.preventDefault(); //console.log('---');
  ...
  touchDiv.addEventListener('touchend', function (e) {
    ...
    touchOK = Math.abs(distX) >= threshold && Math.abs(distY) <= 50; //console.log(distX, distY);
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunktiktakby"] = self["webpackChunktiktakby"] || [];
