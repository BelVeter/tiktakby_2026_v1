...
if ($q_type=='first_step') {
	...
	$q_items = "SELECT * FROM tovar_rent_items WHERE model_id='$model_id' AND `status` IN ('to_rent', 'rented_out') AND `state`!=3 ORDER BY item_rost1, item_rost2";
...
$q_items = "SELECT * FROM tovar_rent_items WHERE model_id='$model_id' AND item_rost1='".$size['item_rost1']."' AND item_rost2='".$size['item_rost2']."' AND `status` IN ('to_rent', 'rented_out') AND `state`!=3 ORDER BY item_rost1";
$result_items = $mysqli->query($q_items);
if (!$result_items) {die('Сбой при доступе к базе данных: '.$q_items.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);}
$items_rows=$result_items->num_rows;
...
<p>Номер вашей брони: <strong>'.$tov_n_1.'-'.$tov_n_2.'</strong>(запомните, запишите или сфотографируйте этот номер - это ВАЖНО!)</p>
<p>Если вы забронировали костюм более чем за 3 дня до нужной даты – необходимо внести предоплату в размере 100% суточной арендной платы не позднее 3 дней с момента бронирования, в противном случае бронь будет аннулирована.
	...
	<input type="button" value="OK" id="general_cans" onclick="br_close(\\\''.$model_id.'\\\');"/>
			';
...
//проверяем, свободно ли это время, если нет - выдаем сообщение, если свободно - продолжаем
	...
	if ($check_rows>0) {
        ...
        $query_new = "INSERT INTO karn_brons SET inv_n='$inv_n', t_from='$t_from', t_to='$t_to', `status`='in_process', cr_time='".time()."', valid_time='".\bb\KBron::makeValidTimestamp()."'";
...
<input type="button" onclick="bron_ok(\\\'bron_ok\\\', \\\''.$inv_n.'\\\', \\\''.$model_id.'\\\');" value="сохранить бронь" /><input type="button" onclick="bron_cans(\\\'bron_cans\\\', \\\''.$inv_n.'\\\', \\\''.$br_id.'\\\', \\\''.$model_id.'\\\', \\\'1\\\');" value="отмена" />
