...
| `CatController` | Catalog: sections, subsections, categories (a hidden technical category answers 301, see Known Specifics #17) |
| `L3Controller` | Individual product (model) page + order form (models of a hidden technical category answer 301, see #17) |
...
| `Mcp/OperationsController` | `/operations/*` (funnel/timeline/by-category/by-location/deals-by-model) + legacy `/orders/stats`, `/deals/list` |
| `Mcp/InventoryController` | `/inventory/{free-tree,pricing,profitability,utilization,turnover,idle}` |
| `Mcp/PricingController` | `/pricing/{history,snapshot}` — журнал изменений тарифов (`rent_tarif_history`) и восстановление прайса на произвольную дату (строки без событий до даты помечаются `extrapolated`) |
...
- `L3Page` — product page; its recommendations block comes from `Model::getModelIdsArrayForFavoriteTovSlider` (same section, units in status `to_rent`, hidden categories excluded, cached 60 min per model/section/subsection)
- `Search\ProductSearch` — internal product search: MySQL FULLTEXT over `rent_model_web` plus a category-name match; hidden technical categories are excluded
...
4. `php artisan migrate --force` — works, but its output is unreliable (see Known Specifics #13): verify the schema explicitly
...
| System | `migrations`, `users`, `personal_access_tokens`, `site_settings` (key-value: `setting_key`/`setting_value`/`updated_at`/`updated_by`; сейчас один ключ — `delivery_next_day_date`, дата ручного переноса доставки на завтра, читается `bb\classes\DeliverySchedule`) |
| Producers | `producers` (`producer_id`, `name` — the one canonical spelling, UNIQUE; `name_norm`; `logo`; `comment`; `is_active`; `cr_/ch_ time/user_id`). **`tovar_rent.producer` stays a plain string** (52 files read it) — no foreign key; the directory only defines the canonical spelling and the brand logo (`bb\classes\Producer`, spec `docs/superpowers/specs/2026-08-14-producers-directory-design.md`) |
| Search | `search_log` (`created_at`, `ip`, `query`, `results_count`, `user_agent`) — internal-search queries, written by `SearchController` |
| Renewals | `rent_ext_calc` — reference calculation of a mass renewal for a client (`bb/dogovor_new.php`, `bb/ajax_ext_calc.php`). Holds only the **last** calc: a new calc replaces the client's rows wholesale, amounts are informational — real money goes to `rent_sub_deals_act` |
| Staff usage | `bb_page_visits` (`user_id`, `page`, `visited_at`) — written by `PageVisitTracker`, see Known Specifics #16 |
...
9. **Sitemap Generation**: A cron job runs `php artisan sitemap:generate` daily to update `sitemap.xml` (root) and `public/sitemap.xml`. The command iterates through all active catalog categories and products. `GenerateSitemap::collectUrls()` builds the URL list (hidden technical categories are excluded, see #17). Subrazdel/category/model URLs use the canonical chain `sub_razdel.main_razdel_id`, NOT the M:N `razdel_subrazdel` (a subrazdel linked to two razdels was listed twice, the second copy pointing `canonical` at the first); categories that answer 301 onto an alias (`Category::URL_ALIASES`, now the Bioptron category — the alias `/ru/medical-prokat/bioptron` stays in the sitemap) are excluded, and `Category::getUrlForPage()` returns the alias too, so menus, breadcrumbs and `/ru/prokat/{cat}` link straight to it (`tests/Feature/CategoryUrlAliasTest.php`); each URL segment is percent-encoded (`pageUrl()`), so a slug with a space or `&` yields a valid `<loc>`. Guarded by `tests/Feature/SitemapGeneratorTest.php`. Both `sitemap.xml` files are generated artifacts: they are listed in `.gitignore` and NOT tracked (guarded by `tests/Feature/SitemapNotTrackedTest.php`), so `Deploy.php`'s `git reset --hard` no longer reverts a fresh file to a stale committed copy. `Deploy.php` deliberately does not run `sitemap:generate`; a new or missing file appears with the 02:00 schedule or a manual `php artisan sitemap:generate` (0.04 s, DB only). The one-off deploy that untracked the files deleted them from the server, so that deploy needs a manual `sitemap:generate` right after it.
...
13. **`php artisan migrate` works on production — but `Deploy.php`'s output about it is not proof.** It was broken 2026-07-05 → 2026-07-12 (ionCube Loader under CloudLinux made every brand-new migration die with `Cannot declare class X, because the name is already in use`); the owner fixed it on 2026-07-12 and new migrations have applied normally since (verified on prod: batch 33 on 2026-08-27, batch 37 on 2026-09-22). So ship schema changes as ordinary migrations — **do not** apply DDL by hand over SSH and then `INSERT INTO migrations`. Two lessons from those deploys: (a) `Deploy.php` printed `Nothing to migrate.` several times in a row although the migration was applied, and on 2026-08-27 it needed more than one call — **never take that line as proof**; after every deploy that carries a migration, check the schema explicitly (`SHOW TABLES` / `SHOW COLUMNS` and the `migrations` table). (b) The schema can arrive later than the code, so either land the DDL in its own PR ahead of the code that depends on it (27.08 order: #299 → #298), or make the new code survive a missing table/column — migrations idempotent (`Schema::hasTable`/`hasColumn`), first reads wrapped in `try/catch` (see `bb\classes\SiteSetting`). Details: `docs/prod_pending.md` journal, `docs/db_notes.md` (gotcha #7).
...
15. **Delivery promise on product cards — one source of truth.** The `Доставка: ...` line and the `Яндекс-доставка сегодня` line in the L2 card (`resources/views/includes/l2_model_block.blade.php`) are computed by `bb\classes\DeliverySchedule`, never inline in Blade. Own courier runs **Mon–Fri only** with a 17:00 cutoff, so Sat/Sun — and Fri after the cutoff — render `в понедельник`. Staff can drop today's slot early (courier already left) with the toggle on `bb/cur_page2.php`, which POSTs to `bb/cur_delivery_day.php` and stores a **date** in `site_settings.delivery_next_day_date`. The caption under the toggle names who flipped it and when, read back from `updated_by`/`updated_at` (today's entries only) — date-bound so a forgotten toggle clears itself at midnight. Salon opening hours live **only** in `bb\classes\SalonHours` and are rendered from there by the «Наш салон проката» popup (`includes/header.blade.php`), the LocalBusiness JSON-LD (`layouts/app.blade.php`) and the Yandex line's visibility window. Change the hours there and all three follow — do not hardcode them again.
16. **Every `bb/` page runs `bb/classes/PageVisitTracker.php` first.** `bb/.htaccess` sets `php_value auto_prepend_file` to the **absolute prod path** (`/var/www/h149208/data/www/tiktak.by/bb/classes/PageVisitTracker.php`; a relative path breaks pages in subdirectories such as `bb/bron/`). It logs a logged-in staff member's visit to `bb_page_visits` (`user_id`, `page`, `visited_at` — no IP, no query string), skips AJAX requests (`X-Requested-With`) and technical scripts (`bb\classes\PageVisitCatalog::isTechnical`), and swallows its own errors so it can never take a page down. Read via `bb/page_track.php` (owner only, `isDima()`) and MCP `staff/page-visits/{by-user,by-page}`. ⚠️ **Local Docker:** the prod path does not exist there — comment out the prod `auto_prepend_file` line in `bb/.htaccess` and uncomment the Docker one, and **never commit that toggle**: `Deploy.php` does `git reset --hard`, so it would land on prod. Design: `docs/superpowers/specs/2026-09-03-crm-usage-tracking-design.md`.
17. **Hidden technical categories** — `bb\classes\Category::HIDDEN_CATEGORY_REDIRECTS` (`category id => redirect URL`; now `175 => /ru/karnavalnye-kostyumy`, the leftover bucket «К УДАЛЕНИЮ», 10 published models). One list drives everything: recommendations, internal search, the age/producer filters, `sitemap:generate` and the 2GIS price/photo query exclude the category (`Category::hiddenCatIdsSql()`), while `L3Controller` (both product routes) and `CatController::categoryMainPage` answer 301. The redirect is decided by the model's real category, not by the URL, because L3 opens by slug under any prefix. Products are NOT deleted or archived. `rent_model_web.status='not_show'` does NOT close a direct URL (`ModelWeb::getByUrlName` ignores status); only archiving (`ModelArchive`) or having no units does. To hide another bucket, add its id to the list. Details: `docs/db_notes.md` items 20–22, audit context: `docs/seo_404_audit_2026-09-24.md`.
18. **Favicons.** `layouts/app.blade.php` links `/favicon.ico` (16/32/48) and `public/images/favicon-{32,120,192}.png` plus `apple-touch-icon.png` (180); the artwork is the flower mark cut from the raster embedded in `public/svg/logo_main.svg`. `.gitignore` ignores `*.png`/`*.ico`, so these files have explicit `!` exceptions, and `tests/Feature/LayoutIconLinksTest.php` fails if a linked icon is missing or untracked. Google wants a side length that is a multiple of 48 (or SVG); Yandex wants 16/32/120 px with `favicon` in the site root.
19. **Text content lives in the DB, not in templates.** Product-card text is `rent_model_web.main_descr` (editable via the MCP API as `description`, with history in `/pages/history` / `mcp_content_versions`); home and listing texts are in `pages` (`code_block_1/2`, `h1_long_text`; home = `level_code='main'`, id 1, NOT editable via the MCP API — admin or direct UPDATE only). The CSS class `floatLeft` used in old descriptions is not defined anywhere; image files referenced by content (`bb/img/*`, `karnaval/img/*`, `vesy/img/*`) are missing on prod and were never tracked in git. See `docs/db_notes.md` item 23 and `docs/reply_to_seo_agent_404_audit_2026-09-24.md`.
20. **A model with no units answers 404 by design (owner decision 2026-09-25).** `L3Controller` returns `l3_not_found` (404) while `tovar_rent_items` has no row for the model, and 200 again when a unit is added; every listing, search, filter, recommendation and the sitemap already require a unit, so only direct URLs reach such a page. Do not "fix" it with a 200 "out of stock" page or a 301 to the category (soft-404). Facts and traffic figures: `docs/db_notes.md` п. 24.
21. **A catalog page is real only if the whole chain resolves by name.** `MainPage::isRealPage()` is level-aware (category: razdel + subrazdel + category all found; subrazdel: razdel + subrazdel; razdel: razdel) — a `pages` row or a single matching razdel no longer makes an invented URL "real" (it used to answer 200 "Раздел не найден.", a soft-404). When the chain does not resolve, `CatController` (subrazdel and category actions) delegates to `RedirectController::notFound`: 301 to the canonical URL if the last segment is a real model/category/subrazdel/razdel, otherwise 404. Real names in a wrong-but-existing combination (`/ru/prokat-sports/kolyaski-detskie/...`) still render 200 with `canonical` to the real chain. Guarded by `tests/Feature/CatalogUnknownPagesTest.php`.
