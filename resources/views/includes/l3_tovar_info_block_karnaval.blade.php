...
<div class="l3__tovar_karn_info_container">
  ...
  <div class="karn_collateral">
    <span style="width: 100%;">{{$p->translate("Оценочная стоимость")}}: ${{number_format($p->getItemDogPrice(),0)}} ({{$p->translate("без учета износа")}})</span>
  </div>
...
{{--          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" id="modal_close_x"></button>--}}
{{--        </div>--}}
        <div class="modal-body">
          <div class="k_first-step_container">
            ...
            <div class="name-div">
              ...
              <div type="button" class="btn-close2" data-bs-dismiss="modal" aria-label="Close" id="modal_close_x"><img src="/public/svg/l3_modal_cross.svg" alt="close modal"></div>
            </div>
            <div class="size-div">
              ...
              <ul class="size-list">
                @if(is_array($rsArray) && count($rsArray)>0)
                  @foreach($rsArray as $rs)
                    <li class="rostsize-li" data-rost_from="{{$rs[0]}}" data-rost_to="{{$rs[1]}}" data-size="{{$rs[2]}}">{{$rs[1]}}</li>
                  @endforeach
                @endif
              </ul>
            </div>
