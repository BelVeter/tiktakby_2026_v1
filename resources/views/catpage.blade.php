...
@section('content')
    ...
    @else
        ...
        <div class="col-12 container-app"><h1 class="h1-listing text-left">{{$p->getH1()}}</h1></div>
    @endif
    ...
    <div class="container-app">
        ...
        <div class="col-12 breadcrumb-line breadcrumb-std-desctop">
            <div class="row">
                ...
                <div class="col-3 ml-auto l2-results-count">РЕЗУЛЬТАТЫ: {{ $p->getStartListingNumber() }}-{{ $p->getEndListingNumber()   }} ИЗ {{$p->getTotalModelsNum()}}</div>
            </div>
        </div>
        ...
        <div class="col-12 cat-main-container">
            ...
            <div class="cat-content-container">
              @if($p->getShowPageNumber()>1)
                <div class="more-btn-div"><a class="l2-more-btn" href="{{\Illuminate\Support\Facades\URL::current().($p->getShowPageNumber()>2 ? '?page='.($p->getShowPageNumber()-1).(request()->gender == '' ? '' : '&gender='.request()->gender).(request()->rost=='' ? '' : '&rost='.request()->rost).(request()->date == '' ? '' : '&date='.request()->date) : '')}}">Показать предыдущие</a></div>
              @endif
                        ...
                        <div class="col">
                            ...
                            @else
                              @if(request()->rost>0 || request()->gender!='' || request()->date!='')
                                <div class="alert-warning"><h2>Товары в данной категории с заданными параметрами фильтрации не найдены. <br> Попробуйте изменить параметры фильтров.</h2></div>
                              @else
                ...
                <div class="listing-end-results">Показано {{ $p->getStartListingNumber() }}-{{ $p->getEndListingNumber()   }} из {{$p->getTotalModelsNum()}}</div>
                @if($p->hasNextModelsButton())
                  <div class="more-btn-div"><a class="l2-more-btn" href="{{\Illuminate\Support\Facades\URL::current().'?page='.($p->getShowPageNumber()+1).(request()->gender == '' ? '' : '&gender='.request()->gender).(request()->rost=='' ? '' : '&rost='.request()->rost).(request()->date == '' ? '' : '&date='.request()->date)}}">Показать еще</a></div>
                @endif
