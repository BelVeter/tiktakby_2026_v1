...
if (isset($_POST['action'])) {
	...
	case 'обновить информацию': // обновление информации + сохранение выезда в случае первой выдачи и продления
		...
		if (!$result_sub_dl) die('Сбой при доступе к базе данных: ' . $query_sub_dl . ' (' . $mysqli->connect_errno . ') ' . $mysqli->connect_error);
		...
		if (!$result_dl_def1) die('Сбой при доступе к базе данных: ' . $query_dl_def1 . ' (' . $mysqli->connect_errno . ') ' . $mysqli->connect_error);
...
if (isset($sort_order)) {
  if ($sort_order=='fio') usort($dls, 'cmpFio');
  elseif ($sort_order=='address') usort($dls, 'cmpAddr');
  else usort($dls, 'cmp');
...
foreach ($dls as $dl) {
    ...
    if ($cur_show=='all') {
        if (!$dl->isCurSubDeal()) continue; //only courier operations to be showed
    }
    else{
        if ($dl->status_sub_prev!=$cur_show) continue;//skip line if filter is on
    }
