...
';
	...
	<style>
		...
		/* Assuming Gilroy is available via CDN or local, using a common CDN for example or just setting the stack */
		/* Note: Gilroy is often a paid font, so 'Gilroy Light' might not work without a proper source. 
					   I will add it to the stack. If the user has it locally or linked, it will pick it up. 
					   Otherwise, I'll keep Montserrat as fallback or use a similar free alternative if requested. 
					   For now, I will put 'Gilroy Light', 'Gilroy' at the front. */
		@import url('https://fonts.googleapis.com/css2?family=Montserrat:wght@300;400;500&display=swap');

		.airy-form {
			font-family: 'Gilroy Light', 'Gilroy', 'Montserrat', sans-serif;
			font-size: 18px;
			/* Updated base size */
		...
		.airy-table tr:last-child td {
			border-bottom: none;
		}

		.airy-table tr:hover td {
			background-color: #f9f9f9;
		}

		.airy-table input[type="submit"],
		.airy-table input[type="button"] {
			padding: 5px 15px;
			border: 1px solid #ccc;
			background-color: #fff;
			border-radius: 15px;
		...
		.airy-table input[type="submit"]:hover,
		.airy-table input[type="button"]:hover {
			border-color: #00adef;
			color: #00adef;
		}
	</style>
...
if (!$client_id > 0) {  // форма поиска клиентов
	...
	echo '
<div class="airy-form" id="client_find_div">
	<div class="airy-header">Найти клиента</div>
	<form name="poisk" method="post" action="dogovor_new4.php">
		<div class="airy-row">
			<div class="airy-input-group">
				<label>№ клиента</label>
				<input type="text" name="s_client_id" class="airy-input" value="' . $s_client_id . '" />
			</div>
			<div class="airy-input-group">
				<label>№ паспорта</label>
				<input type="text" name="s_pas_n" class="airy-input" value="' . $s_pas_n . '" />
			</div>
			<div class="airy-input-group">
				<label>Телефон</label>
				<input type="text" name="s_ph" class="airy-input" value="' . $s_ph . '" />
			</div>
		</div>
		<div class="airy-row">
			<div class="airy-input-group">
				<label>Фамилия</label>
				<input type="text" name="s_family" class="airy-input" value="' . $s_family . '" />
			</div>
			<div class="airy-input-group">
				<label>Имя</label>
				<input type="text" name="s_name" class="airy-input" value="' . $s_name . '" />
			</div>
			<div class="airy-input-group">
				<label>Отчество</label>
				<input type="text" name="s_otch" class="airy-input" value="' . $s_otch . '"/>
			</div>
		</div>
		<div class="airy-row">
			<div class="airy-input-group" style="flex: 2;">
				<label>Улица</label>
				<input type="text" name="s_str" class="airy-input" value="' . $s_str . '" />
			</div>
			<div class="airy-input-group" style="flex: 0 0 80px;">
				<label>Дом</label>
				<input type="text" name="s_dom" class="airy-input" value="' . $s_dom . '" />
			</div>
			<div class="airy-input-group" style="flex: 0 0 80px;">
				<label>Кв</label>
				<input type="text" name="s_kv" class="airy-input" value="' . $s_kv . '" />
			</div>
		</div>
		<div class="airy-row" style="justify-content: flex-end;">
			<input type="submit" name="action" value="найти" class="airy-btn airy-btn-primary" /> 
			<button class="airy-btn" onclick="cl_displ (); return false;">Завести нового клиента</button>
		</div>
...
<tr>
	...
	while ($client_list = $result_cl->fetch_assoc()) {
		...
		echo '
	<form name="client_s_' . $client_list['client_id'] . '" method="post" action="dogovor_new4.php" >
	...
	if ($rows_cl == 0)
		echo '<div class="airy-form" style="text-align: center; color: #dc3545; padding: 20px;">Извините, по Вашему запросу ничего не найдено</div>';
...
echo '
<form name="main" id="main_form" method="post" action="dogovor_new4.php" >

<div class="airy-form" id="client_info_div" ' . ($client_id > 0 ? '' : 'style="display:none;"') . '>
';
...
if ($client_id > 0) {
	echo '<div class="airy-header">Информация о клиенте (№' . $client_id . ') <button class="airy-btn" id="cl_edit_button" onclick="hide_client(); return false;">редактировать</button></div>';
} else {
	echo '<div class="airy-header">Ввести нового клиента ' . (\bb\models\User::getCurrentUser()->isOwner() ? '<button class="airy-btn" onclick="fill_placeholders(); return false;">Заполнить плейсхолдерами</button>' : '') . '</div>';
}
...
<div class="airy-row">
	<div class="airy-input-group">
		<label>Фамилия</label>
		<input type="text" name="family" id="family" class="airy-input" ' . $d_disabled . ' value="' . $cl_def['family'] . '" />
	</div>
	<div class="airy-input-group">
		<label>Имя</label>
		<input type="text" name="name" id="name" class="airy-input" ' . $d_disabled . ' value="' . $cl_def['name'] . '" />
	</div>
	<div class="airy-input-group">
		<label>Отчество</label>
		<input type="text" name="otch" id="otch" class="airy-input" ' . $d_disabled . ' value="' . $cl_def['otch'] . '" />
	</div>
</div>

<div class="airy-sub-header">Адрес</div>
<div class="airy-row">
	<div class="airy-input-group" style="flex: 2;">
		<label>Улица</label>
		<input type="text" name="str" id="str" class="airy-input" ' . $d_disabled . ' value="' . $cl_def['str'] . '" />
	</div>
	<div class="airy-input-group" style="flex: 0 0 80px;">
		<label>Дом</label>
		<input type="text" name="dom" id="dom" class="airy-input" ' . $d_disabled . ' value="' . $cl_def['dom'] . '" />
	</div>
	<div class="airy-input-group" style="flex: 0 0 80px;">
		<label>Кв</label>
		<input type="text" name="kv" id="kv" class="airy-input" ' . $d_disabled . ' value="' . $cl_def['kv'] . '" />
	</div>
	<div class="airy-input-group">
		<label>Город</label>
		<input type="text" name="city" id="city" class="airy-input" ' . $d_disabled . ' value="' . $cl_def['city'] . '" />
	</div>
	<button class="airy-btn" id="address_copy" ' . $d_disabled . ' onclick="copy_addr(); return false;">Копировать в прописку</button>
</div>

<div class="airy-sub-header">Прописка</div>
<div class="airy-row">
	<div class="airy-input-group" style="flex: 2;">
		<label>Улица</label>
		<input type="text" name="reg_str" id="reg_str" class="airy-input" ' . $d_disabled . ' value="' . $cl_def['reg_str'] . '" />
	</div>
	<div class="airy-input-group" style="flex: 0 0 80px;">
		<label>Дом</label>
		<input type="text" name="reg_dom" id="reg_dom" class="airy-input" ' . $d_disabled . ' value="' . $cl_def['reg_dom'] . '" />
	</div>
	<div class="airy-input-group" style="flex: 0 0 80px;">
		<label>Кв</label>
		<input type="text" name="reg_kv" id="reg_kv" class="airy-input" ' . $d_disabled . ' value="' . $cl_def['reg_kv'] . '" />
	</div>
	<div class="airy-input-group">
		<label>Город</label>
		<input type="text" name="reg_city" id="reg_city" class="airy-input" ' . $d_disabled . ' value="' . $cl_def['reg_city'] . '" />
	</div>
</div>
...
<div class="airy-row">
	<div class="airy-input-group">
		<label>№ паспорта</label>
		<input type="text" name="pas_n" id="pas_n" class="airy-input" ' . $d_disabled . ' value="' . $cl_def['pas_n'] . '" />
	</div>
	<div class="airy-input-group">
		<label>Дата выдачи</label>
		<input type="date" name="pas_date" id="pas_date" class="airy-input" ' . $d_disabled . ' value="' . $cl_def['pas_date'] . '" />
	</div>
	<div class="airy-input-group" style="flex: 2;">
...
<div class="airy-row">
	...
	<div class="airy-input-group">
		...
		<input type="text" name="phone_2" id="phone_2" class="airy-input" ' . $d_disabled . ' value="' . phone_print($cl_def['phone_2']) . '" />
	</div>
</div>

<div class="airy-input-group" style="width: 100%;">
	<label>Дополнительная информация</label>
	<textarea name="info" id="info" class="airy-input" rows="3" ' . good_print($d_disabled) . '>' . $cl_def['info'] . '</textarea>
</div>
';
...
if ($client_id > 0) {
	echo '
		<div class="airy-input-group">
			...
			<select name="source" ' . $d_disabled . ' id="source_sel" class="airy-input">
	...
	echo '
		<div class="airy-input-group" style="width: 100%;">
			<label>Источник</label>
			<div class="radio-toolbar" id="source_radio_div" style="margin-top: 10px;">
...
<?php
echo '
<div class="airy-row" style="justify-content: center; margin-top: 20px;">
	<input type="submit" name="action" id="action_save_cl" class="airy-btn airy-btn-primary" ' . ($client_id > 0 ? 'value="обновить информацию только по клиенту" disabled="disabled"' : 'value="сохранить только клиента"') . ' onclick="return form_check_cl();" /> 
</div>
</div>
';
...
echo '</select>
	...
	<input type="button" value="Отмена" onclick="location.href=\'/bb/dogovor_new4.php\';" /></div>';
...
echo '<div id="print_buttons" ' . $pr_displ . '>
				...
				<input type="button" name="action" id="action_print" value="новый договор/сделка" onclick="location.href=\'/bb/dogovor_new4.php\';" />
			</div>';
