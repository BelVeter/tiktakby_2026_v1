...
<tr style="font-weight:bold;">
	...
	<table border="1" cellspacing="0" style="clear:both;">
		...
		'<th style="width:60px;">уч.дата</th>
			...
			<th style="width:90px;">Даты сделки<br />период</th>
...
$prev_date=$sub_dl_def['acc_date'];
	...
	echo'	>'.($_SESSION['user_id']==3 ? '<td>'.$sub_dl_def['deal_id'].'<br />'.$sub_dl_def['sub_deal_id'].'</td>' : '').
			'<td>'.date("d.m.y", $sub_dl_def['acc_date']).'<br /><i>('.date("H:i", $sub_dl_def['cr_time']).')</i></td>
			...
			<td> <strong>№'.inv_print($dl_def['item_inv_n']).'</strong><br />'.$cat_def['dog_name'].' '.$model_def['model'].', '.$model_def['producer'].$color.'</td>
			<td>'.date("d.m.y", $dl_def['start_date']).' - '.date("d.m.y", $dl_def['return_date']).'</td>
...
function kassa ($pole, $kassa, $place, $sort_date) {
	...
	$query_r_of_nal_no_check1 = "SELECT SUM($pole) FROM rent_sub_deals_act WHERE `type` IN ('payment', 'cl_payment') AND $p_type='$kassa' ".$pl_srch."AND $sort_date";
	...
	$query_r_of_nal_no_check2 = "SELECT SUM($pole) FROM rent_sub_deals_arch WHERE `type` IN ('payment', 'cl_payment') AND $p_type='$kassa' ".$pl_srch."AND $sort_date";
...
function kassa_n_r ($pole, $kassa, $place, $sort_date, $deal_id_rash) {
	...
	$query_r_of_nal_no_check1 = "SELECT SUM($pole) FROM rent_sub_deals_act WHERE `type` IN ('payment', 'cl_payment') AND deal_id!='$deal_id_rash' AND $p_type='$kassa' ".$pl_srch."AND $sort_date";
	$result_r_of_nal_no_check1 = mysql_query($query_r_of_nal_no_check1);
	if (!$result_r_of_nal_no_check1) die("Сбой при доступе к базе данных: '$query_r_of_nal_no_check1'".mysql_error());
	$r_pay_act=mysql_fetch_array($result_r_of_nal_no_check1);

	$query_r_of_nal_no_check2 = "SELECT SUM($pole) FROM rent_sub_deals_arch WHERE `type` IN ('payment', 'cl_payment') AND deal_id!='$deal_id_rash' AND $p_type='$kassa' ".$pl_srch."AND $sort_date";
	$result_r_of_nal_no_check2 = mysql_query($query_r_of_nal_no_check2);
	if (!$result_r_of_nal_no_check2) die("Сбой при доступе к базе данных: '$query_r_of_nal_no_check2'".mysql_error());
	$r_pay_arch=mysql_fetch_array($result_r_of_nal_no_check2);
