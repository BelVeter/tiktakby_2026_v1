...
if ($q_type=='br_start') {
		...
		$output.='Сожалеем. В настоящий момент данного товара нет в наличии. <br />Однако, Вы можете оставить заявку.<br />Как только товар освободится наш менеджер свяжется с Вами. <br /><br />
			<table>
				...
				<tr>
					...
					<td>+375 <input type="text" style="width:25px;" id="tel_pr" /><input type="text" id="tel" style="width:70px;" /> </td>
				</tr>
				<tr>
				...
				<input type="button" value="Оставить заявку" onclick="br_zayav(\\\''.$model_id.'\\\');" /> 
	...
	elseif ($free1>0 || $free2>0 || $free3>0) {
		...
		$output.='
				'.$model_def['item_name_main'].' в наличии по адресу:<br />
						'.($free1>0 ? '- ул. Литературная 22 <br />' : '').'
						'.($free2>0 ? '- ул.Ложинская 5 (ст. м. "Уручье")' : '').' 
						'.($free3>0 ? '- пр-т. Победителей 127 (район Минск-Арена)' : '').' 
		...
		if ($free3>0) {
			$output.='<br><input type="radio" name="br_of_radio" id="br_of_radio_3"><label for="br_of_radio_3">Победителей 127</label> ';
		}
			...
			$output.='	
				...
				<br /><input type="button" value="Заказать" onclick="br_office(\\\''.$model_id.'\\\');" /> 
				</div>
...
if ($br_sposob=='office') {	//для самозабора
	...
	if ((($free1_def['br_time']==$br_time) || ($free1_def['br_time']<time()))) {//проверяем, по времени, если время наше, либо чужое но уже прошедшее то бронируем
		...
		$info=$fio.($br_sposob=='deliv' ? '<br />Адрес доставки:'.$deliv_addr.'<br />' : '').'+375-'.$tel_pr.'-'.$tel.'<br />'.$dop_info;
		
		$br_sposob=='deliv' ? $deliv_yn=1 : $deliv_yn=0;
		$br_sposob=='deliv' ? $type_2_q='deliv' : $type_2_q='bron';
		
		$query = "INSERT INTO rent_orders VALUES ('', 'strong', '$ac_date', '', '', '$validity', '".$free1_def['item_inv_n']."', '".$free1_def['model_id']."', '".$free1_def['cat_id']."', '$type_2_q', '', '$info', '1', '".time()."', '', '', '', '', '', '', '".$_SERVER['REMOTE_ADDR']."', '', '')";
		...
		if ($br_sposob=='office') {
			if ($free1_def['item_place']==1) {
				$dop_out='<p>Товар можно забрать по адресу: ул. Литературная 22</p></p> График работы: <br /> Пн-пт: 10:00 - 19:00 <br /> Сб.,Вс: выходной <br /></p>
					<p>Телефоны:<br />+375(29) 630-35-32 <br />
					+375(29) 730-35-34 </p>';
			}
			elseif ($free1_def['item_place']==2) {
				$dop_out='<p>Товар нужно забрать по адресу: ул.Ложинская 5 (ст. м. "Уручье")</p><p>График работы: <br /> Пн-пт: 10:00 - 18:00 <br /> Сб,Вс: 11:00-15:00</p>
					<p>Телефоны:<br />+375(29) 630-35-58 <br />
					+375(29) 735-35-95</p>';
			}
			else {//третий офис
				$dop_out='<p>Товар нужно забрать по адресу: пр-т Победителй 127 (р-н Минск-Арена)</p><p>График работы: <br /> Пн-пт: 10:00 - 18:00 <br /> Сб: 10:00-15:00<br>Вс: выходной</p>
					<p>Телефоны:<br />+375(29) 630-35-58 <br />
					+375(29) 735-35-95</p>';
			}
		}
		...
		if ($free_zap_n>0) {
			...
			$query = "INSERT INTO rent_orders VALUES ('', 'strong', '$ac_date', '', '', '$validity', '".$free_zap['item_inv_n']."', '".$free_zap['model_id']."', '".$free_zap['cat_id']."', '$type_2_q', '', '$info', '1', '".time()."', '', '', '', '', '', '', '".$_SERVER['REMOTE_ADDR']."', '', '')";
			$result = $mysqli->query($query);
			if (!$result) {die('Сбой при доступе к базе данных: '.$query.' ('.$mysqli->connect_errno.') '.$mysqli->connect_error);}
			...
			if ($br_sposob=='office') {
				...
				elseif ($free1_def['item_place']==2) {
					...
					$dop_out='<p>Товар нужно забрать по адресу: пр-т Победителй 127 (р-н Минск-Арена)</p><p>График работы: <br /> Пн-пт: 10:00 - 20:00 <br /> Сб, Вс: 11:00-18:00</p>
						<p>Телефоны:<br />+375(29) 630-35-58 <br />
						+375(29) 735-35-95</p>';
				}
			}
...
if ($q_type=='sam_zayav') {
	...
	$info=$fio.'<br /> +375-'.$tel_pr.'-'.$tel.'<br />'.$dop_info;
	...
	$query = "INSERT INTO rent_orders VALUES ('', 'zayavka', '$ac_date', '', '', '$validity', '', '$model_id', '".$model_m['tovar_rent_cat_id']."', 'zayavka', '', '$info', '1', '".time()."', '', '', '', '', '', '', '".$_SERVER['REMOTE_ADDR']."', '', '')";
	$result = $mysqli->query($query);
	if (!$result) {die('Сбой при доступе к базе данных: '.$query.' ('.$mysqli->connect_errno.') '.$mysqli->connect_error);}
