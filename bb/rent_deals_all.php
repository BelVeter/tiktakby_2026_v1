...
if (!isset($_POST['action']) && isset($_POST['prev_action'])) {
	$_POST['action'] = $_POST['prev_action'];
}

if (!isset($_POST['action'])) {
	$_POST['action'] = 'сегодня';
}
...
if ($ch_num_update == 'yes') {
	...
	if (!$result_aa)
		die("Сбой при доступе к базе данных: '$q_arch_act'" . mysql_error());
...
if (isset($_POST['action'])) {
	...
	if ($action == 'сохранить остаток') {
		...
		if (!mysql_query($sub_query, $db_server)) {
			$done = "no";
			echo "Сбой при вставке данных: '$sub_query' <br />" . mysql_error() . "<br /><br />";
		}
...
if (!$result_lp)
	die("Сбой при доступе к базе данных: '$rd_lp'" . mysql_error());
...
$result_cats = mysql_query($query_cats);
if (!$result_cats)
	die("Сбой при доступе к базе данных: '$query_cats'" . mysql_error());
...
if ($no_action != 1) {
	...
	if (!$result_sub_dl_def)
		die("Сбой при доступе к базе данных: '$query_sub_dl_def'" . mysql_error());
	...
	if (!$result_item_r)
		die("Сбой при доступе к базе данных: '$query_item_r'" . mysql_error());
	$item_r = mysql_fetch_array($result_item_r);
	$item_r_rows = mysql_num_rows($result_item_r);
	$r_dl_id = $item_r['active_deal_id'];

} else {
	die('<br /><br /><font size="4">Выберите период для начала работы.</font>');
}
...
if (!$result_k1)
	die("Сбой при доступе к базе данных: '$query_k1'" . mysql_error());
...
if (!$result_k2)
	die("Сбой при доступе к базе данных: '$query_k2'" . mysql_error());
...
if (!$result_k1_now)
	die("Сбой при доступе к базе данных: '$query_k_now'" . mysql_error());
...
if (!$result_ri)
	die("Сбой при доступе к базе данных: '$ri_q'" . mysql_error());
...
if (!$result_rd)
	die("Сбой при доступе к базе данных: '$rd_q'" . mysql_error());
...
if (!$result_dr)
	die("Сбой при доступе к базе данных: '$dr_q'" . mysql_error());
...
while ($sub_dl_def = mysql_fetch_array($result_sub_dl_def)) {
	...
	if ($sub_dl_def['type'] == 'payment' || $sub_dl_def['type'] == 'cl_payment') {
		...
		if (!$result_sub_1)
			die("Сбой при доступе к базе данных: '$query_sub_1'" . mysql_error());
	...
	if (!$result_dl_def)
		die("Сбой при доступе к базе данных: '$query_dl_def'" . mysql_error());
	...
	if (!$result_cl_def)
		die("Сбой при доступе к базе данных: '$query_cl_def'" . mysql_error());
	...
	if (!$result_item_def)
		die("Сбой при доступе к базе данных: '$query_item_def'" . mysql_error());
	...
	if (!$result_fr)
		die("Сбой при доступе к базе данных: '$query_fr'" . mysql_error());
	...
	if ($sub_dl_def['type'] == 'close') {
		...
		if (!$result_ret_d)
			die("Сбой при доступе к базе данных: '$query_ret_d'" . mysql_error());
	...
	if (!$result_model_def)
		die("Сбой при доступе к базе данных: '$query_model_def'" . mysql_error());
	...
	if (!$result_cat_def)
		die("Сбой при доступе к базе данных: '$query_cat_def'" . mysql_error());
	...
	if ($sub_dl_def['type'] != 'payment' && $sub_dl_def['type'] != 'cl_payment') {
		...
		if (!$result_sub_pay)
			die("Сбой при доступе к базе данных: '$query_sub_pay'" . mysql_error());
		...
		if ($fr['place'] == 1 && $fr['delivery_yn'] != 1) {
			$fr_chanell = '<img src="/bb/assets/images/1.gif" style="position:absolute; right:0; top:0; width:25px; heght:25px;" />';
		} elseif ($fr['place'] == 2 && $fr['delivery_yn'] != 1) {
			$fr_chanell = '<img src="/bb/assets/images/2.gif" style="position:absolute; right:0; top:0; width:25px; heght:25px;" />';
		} elseif ($fr['place'] == 3 && $fr['delivery_yn'] != 1) {
			$fr_chanell = '<img src="/bb/assets/images/3.gif" style="position:absolute; right:0; top:0; width:25px; heght:25px;" />';
		} elseif ($fr['delivery_yn'] == 1) {
			$fr_chanell = '<img src="/bb/assets/images/k.png" style="position:absolute; right:0; top:0; width:25px; heght:25px;" />';
		} else {
			$fr_chanell = '';
		}
	...
	if ($sub_dl_def['courier_id'] > 0) {
		...
		if (!$result_cur)
			die("Сбой при доступе к базе данных: '$query_cur'" . mysql_error());
...
function kassa($pole, $kassa, $place, $sort_date)
{
	...
	if (!$result_r_of_nal_no_check1)
		die("Сбой при доступе к базе данных: '$query_r_of_nal_no_check1'" . mysql_error());
	...
	if (!$result_r_of_nal_no_check2)
		die("Сбой при доступе к базе данных: '$query_r_of_nal_no_check2'" . mysql_error());
...
function rash_sum($kassa)
{
	...
	if (!$result_drs)
		die("Сбой при доступе к базе данных: '$dr_s'" . mysql_error());
