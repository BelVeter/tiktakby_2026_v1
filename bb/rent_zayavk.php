...
function show_edit(id) {
	...
	if (document.getElementById('edit_show_'+id).value=="оформить звонок") {
	...
	document.getElementById('edit_show_'+id).value="оформить звонок";
...
if (isset($_POST['action'])) {
	...
	switch ($action) {
		...
		case 'самовывоз':
			...
			if ($br->ch_time!=$last_ch_time) {die('Бронь была изменена кем-то другим. Повторите операцию (только на этот раз не тормозите :)).');}
...
while ($ord = $result_or->fetch_assoc()) {
	...
	//поиск свободных товаров
		...
		if (!$res_f) {die('Сбой при доступе к базе данных: '.$query_f.' ('.$mysqli->connect_errno.') '.$mysqli->connect_error);}
		...
		<td>
			...
			<div>
				...
				<input type="hidden" name="type2" id="type2_'.$br_line->order_id.'" value="'.$br_line->type2.'">
    	  		...
    	  		<input type="button"  name="action" id="edit_show_'.$br_line->order_id.'" value="оформить звонок" onclick="show_edit(\''.$br_line->order_id.'\');">
				<input type="submit" style="display:none;" name="action" id="save_podtv_'.$br_line->order_id.'" value="сохранить звонок" style="display:none;">
				<input type="submit" style="background-color:#ed8886;" name="action" id="del_but_'.$br_line->order_id.'" id="action_'.$br_line->order_id.'" onclick="return confirm(\'Вы точно хотите удалить эту бронь?\');" value="удалить"><br />
