...
if (1==1) {    //if (isset($action) && $action=='поделить плохие долги') {
    ...
    if (!$bad_d_result) die( 'Сбой при доступе к базе данных: ' . $bad_d_q . ' (' . $mysqli->connect_errno . ') ' . $mysqli->connect_error);
    ...
    if ($bad_d_result->num_rows > 0) {
        ...
        if (!$bad_d_result2) die( 'Сбой при доступе к базе данных: ' . $bad_d_result2 . ' (' . $mysqli->connect_errno . ') ' . $mysqli->connect_error);

        //заполняем массив уже поделенных клиентов
        $prev_client_id='';
        while ($bad_d2=$bad_d_result2->fetch_assoc()) {
            if (!array_key_exists($bad_d2['deal_status'], $bad_solvers_count)) continue; //if not active user - skip
            $bad_solvers_clients[$bad_d2['client_id']]=$bad_d2['deal_status']; //for every client user is assigned. If duplication - just overwright :).
            if ($prev_client_id!=$bad_d2['client_id']) $bad_solvers_count[$bad_d2['deal_status']]+=1; //increase counter
        ...
        while ($bad_d=$bad_d_result->fetch_assoc()) {
            ...
            if (isset($bad_solvers_clients[$bad_d['client_id']])) $min_user_id=$bad_solvers_clients[$bad_d['client_id']];
            else $min_user_id=array_keys($bad_solvers_count, min($bad_solvers_count))[0];  # array('$bad_solvers_count')
...
while ($dl_def=$result_dl_def->fetch_assoc()) {
    ...
    if (!$result_item_def) die( 'Сбой при доступе к базе данных: ' . $query_item_def . ' (' . $mysqli->connect_errno . ') ' . $mysqli->connect_error);
    ...
    if (!$result_cat_def) die( 'Сбой при доступе к базе данных: ' . $query_cat_def . ' (' . $mysqli->connect_errno . ') ' . $mysqli->connect_error);
    ...
    if (!$result_fr_def) die( 'Сбой при доступе к базе данных: ' . $query_fr_def . ' (' . $mysqli->connect_errno . ') ' . $mysqli->connect_error);
		$fr_def = $result_fr_def->fetch_assoc();

    //office filter
    if ($office_filter !='0' ) {
      if($fr_def['place']!= $office_filter*1 || $fr_def['delivery_yn']==1) continue;
    }
	...
	if ($fr_def['place']=='1') {
		$of_pic='оф1<img src="/images/1.gif" alt="офис 1" style="position:absolute; top:0px; right:0px;" />';
	}
	elseif ($fr_def['place']=='2') {
		$of_pic='оф2<img src="/images/2.gif" alt="офис 2" style="position:absolute; top:0px; right:0px;" />';
	}
