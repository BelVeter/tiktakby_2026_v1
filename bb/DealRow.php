...
//echo '</pre>';
        ...
        if ($sub_type!='all') {
            foreach ($subs as $key => $value) {
                if ($subs[$key]->type_sub_deal!=$sub_type) unset($subs[$key]);
            }
        }
        if ($payment_type!='all') {
            foreach ($subs as $key => $value) {
                if (($subs[$key]->r_paid+$subs[$key]->del_paid)==0) unset($subs[$key]);
            }
        }
...
//                    }
                ...
                if ($payment_type!='all') {
                    if($p->r_payment_type!=$payment_type) $p->r_paid=0;
                    if ($p->del_payment_type!=$payment_type) $p->delivery_paid=0;
                    if (($p->r_paid+$p->delivery_paid)>0) $pays[]=$p;
                }
                else $pays[]=$p;
            }
        }
...
//                    echo '</pre>';
    ...
    public static function GetSubDeals($sub_id=-1, $table, $date='', $place='', $cur='no_filter') {
        ...
        if ($sub_id==-1) {//main request
            ...
            if ($place!='all') $where_close.=" AND sub1.`place`=$place";
...
//        echo '<br>'.$sql_query.'<br><br>';
    ...
    public static function GetLine($sub_id) {
        ...
        if ($result->num_rows<1) $result = self::GetSubDeals($sub_id, 'arch');
...
//                break;
    ...
    public function FirstPlacePic() { //do not forgea about arch and act
        if (!($this->first_rent_place>1)) {
            ...
            if ($row['place']>0) $this->first_rent_place=$row['place'];
        }
    ...
    public function LastExtensionDatePrint() {
        if ($this->type_sub_deal=='close' || $this->type_sub_deal=='cur_return') {
            if (!($this->to_sub_deal>0)) {
                ...
                $result = $mysqli->query($sql_query);
                $row = $result->fetch_assoc();
                if ($row['to']>0) $this->to_sub_deal=$row['to'];
            }
    ...
    public function PrintPayment($form_print='yes') {
        ...
        foreach ($this->payments as $payment) {
            ...
            if($payment->r_paid != 0) $output .= '<strong>' . number_format($payment->r_paid, 2, ',', ' ') . $payment->KassaPrint($payment->r_payment_type) . '</strong>';
            if ($payment->delivery_paid != 0) $output .= '<span class="deliv_num">' . number_format($payment->delivery_paid, 2, ',', ' ') . '</span>';
