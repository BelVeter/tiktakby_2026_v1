...
document.querySelector('.own-rez-btn').addEventListener('click',showNoOwnResultRow);

function showNoOwnResultRow(){
  document.querySelector('.no-own-zpl-rez').classList.toggle('hide');
}
...
function cellClick(e){
  ...
  let from = new Date();
    from.setFullYear(year, month*1-1,1);
  ...
  var data = {
    action: 'показать',
    i_from_date: from.getFullYear()+'-'+((from.getMonth()*1+1)>9 ? (from.getMonth()*1+1) : '0'+(from.getMonth()*1+1))+'-'+(from.getDate() > 9 ? from.getDate() : '0'+from.getDate()),
    i_to_date: toText,
    item_place: 'all',
    kassa_s: 'all',
    type1_s: type1,
