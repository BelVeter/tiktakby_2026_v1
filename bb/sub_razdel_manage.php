...
if (isset($_GET['razdel_id'])) $razdel_id = \bb\Base::getGet('razdel_id');
if (isset($_GET['subrazdel_id'])) $subrazdel_id = \bb\Base::getGet('subrazdel_id');

if (isset($_POST['action'])) {
    switch ($_POST['action']){
        case 'save_new':
            $sr = new \bb\classes\SubRazdel();
            if (isset($_POST['id_sub_razdel'])) $sr->setIdSubRazdel( $_POST['id_sub_razdel']);
            ...
            if (key_exists('url_sub_razdel_icon', $_FILES) && $_FILES['url_sub_razdel_icon']['name'] != '') {
                ...
                if (key_exists('url_sub_razdel_icon', $_FILES)) {
                    ...
                    move_uploaded_file($_FILES['url_sub_razdel_icon']['tmp_name'], $dir_full.$newFileName);
                    $sr->setUrlSubRazdelIcon($dir_short.$newFileName);
                }
                //delete old file if exists
                if ($sr->getIdSubRazdel()>0) {
                    $old_sr = \bb\classes\SubRazdel::getById($sr->getIdSubRazdel());
                    if ($old_sr->getUrlSubRazdelIcon()!='') {
                        if(!unlink($_SERVER['DOCUMENT_ROOT'].$old_sr->getUrlSubRazdelIcon())){
...
//            \bb\Base::varDamp($sr);
        ...
        case 'delete':
            ...
            if ($sr->getIdSubRazdel()>0 && $sr->getUrlSubRazdelIcon() != '' && is_file($_SERVER['DOCUMENT_ROOT'].$sr->getUrlSubRazdelIcon())) {
                unlink($_SERVER['DOCUMENT_ROOT'].$sr->getUrlSubRazdelIcon());
            }
...
<div style="display: flex; justify-content: flex-end"><button class="btn btn-info new-btn">Ввести новый подраздел</button></div>
<table class="table">
    <thead>
        <tr>
            <th>Разделы<br>
                <select form="srch_form" class="form-control-sm subrazdel-filter" name="razdel_id" <?= ($subrazdel_id>0 ? 'disabled' : '') ?>>
                    ...
                    <?php foreach ($p->getAllRazdels() as $r): ?>
                        <option value="<?= $r->getIdRazdel() ?>" <?= ($razdel_id == $r->getIdRazdel() ? 'selected' : '' ) ?>><?= $r->getNameRazdelText() ?></option>
                    <?php endforeach; ?>
                </select>
            </th>
            <th>id</th>
            <th>Подраздел<br>
                <select form="srch_form" class="form-control-sm subrazdel-filter choices-single" name="subrazdel_id" <?= ($razdel_id>0 ? 'disabled' : '') ?>>
    ...
    <tbody>
        <tr class="new-form-row">
            ...
            <td><input class="form-control" type="text" name="name_sub_razdel_text" form="new_form"><br>
                <input class="form-control" type="text" name="name_sub_razdel_text_en" form="new_form" placeholder="en"><br>
                <input class="form-control" type="text" name="name_sub_razdel_text_lt" form="new_form" placeholder="lt">
            </td>
            <td><input class="form-control" type="text" name="url_sub_razdel_name" form="new_form" data-controll="url"></td>
            <td><input class="form-control" type="file" name="url_sub_razdel_icon" form="new_form" accept=".gif,.jpg,.jpeg,.png,.svg"></td>
            <td><input class="form-control" type="number" step="1" name="order_num_sub_razd" form="new_form" value="0"></td>
            <td>
              <div class="form-check">
                <input class="form-check-input filter_input" type="checkbox" value="1">
                <input title="фильтр" placeholder="фильтр" type="text" class="form-control form-control-sm cat_filter_text">
              </div>
    ...
    <?php foreach ($p->getSubRazdelsAll() as $sr): ?>
        <tr>
            <td>
                <span class="hide-when-edit"><?= $p->getRazdelsString($sr->getBindedRazdelIds(), $sr->getMainRazdelId()) ?></span>
                <?= $p->getMainRazdelSelect('new_form', $sr->getIdSubRazdel(), $sr->getMainRazdelId()) ?>
                <div class="edit-field"><?= $p->getRazdelCheckBoxes('new_form_'.$sr->getIdSubRazdel(), $sr->getBindedRazdelIds(), $sr->getMainRazdelId()); ?></div>
            </td>
            <td><?= $sr->getIdSubRazdel() ?></td>
            <td><?= $sr->getNameSubRazdelText() ?>
                <input class="form-control edit-field" type="text" name="name_sub_razdel_text" form="new_form_<?= $sr->getIdSubRazdel() ?>" value="<?= $sr->getNameSubRazdelText() ?>"><br>
                <input class="form-control edit-field" type="text" name="name_sub_razdel_text_en" form="new_form_<?= $sr->getIdSubRazdel() ?>" value="<?= $sr->getNameSubRazdelText('en', 1) ?>" placeholder="en"><br>
                <input class="form-control edit-field" type="text" name="name_sub_razdel_text_lt" form="new_form_<?= $sr->getIdSubRazdel() ?>" value="<?= $sr->getNameSubRazdelText('lt', 1) ?>" placeholder="lt">
            </td>
            <td><?= $sr->getUrlSubRazdelName() ?>
                 <input class="form-control edit-field" type="text" name="url_sub_razdel_name" form="new_form_<?= $sr->getIdSubRazdel() ?>" <?= (\bb\models\User::getCurrentUser()->hasPermission(5) ? '' : 'readonly="readonly"') ?> value="<?= $sr->getUrlSubRazdelName() ?>" data-controll="url">
            </td>
            <td><img class="form-icon" src="<?= $sr->getUrlSubRazdelIcon() ?>">
                <input class="form-control edit-field" type="file" name="url_sub_razdel_icon" form="new_form_<?= $sr->getIdSubRazdel() ?>"></td>
            <td><?= $sr->getOrderNumSubRazd() ?>
                <input class="form-control edit-field" type="number" step="1" name="order_num_sub_razd" form="new_form_<?= $sr->getIdSubRazdel() ?>" value="<?= $sr->getOrderNumSubRazd() ?>">
            </td>
            <td>
                ...
                <div class="edit-field"><?= $p->getCatCheckBoxes('new_form_'.$sr->getIdSubRazdel(), $sr->getBindedCatIds()); ?></div>
            </td>
            <td>
                <form name="new_form_<?= $sr->getIdSubRazdel() ?>" id="new_form_<?= $sr->getIdSubRazdel() ?>" method="post" enctype="multipart/form-data">
