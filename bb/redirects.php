...
function sortLink($key, $label, $currentSort, $currentDir)
{
    ...
    // Сохраняем текущую страницу или сбрасываем? Обычно при смене сортировки лучше сбрасывать на 1 страницу
...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    border-radius: 6px;
    font-size: 14px;
    background: #fff url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
        fill="%23999" viewBox="0 0 16 16">
        <path
            ...
            <ul class="pagination justify-content-center">
                ...
                for ($i = $start; $i <= $end; $i++): ?>
                                <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                                    <a class="page-link" href="?page=<?= $i ?>&sort=<?= $sortKey ?>&dir=<?= $dir === 'ASC' ? 'asc' : 'desc' ?>"><?= $i ?></a>
                                </li>
                    <?php endfor;
                if ($end < $totalPages)
                    echo '<li class="page-item disabled"><span class="page-link">...</span></li>';
                ?>
                    ...
                    <li class="page-item <?= ($page >= $totalPages) ? 'disabled' : '' ?>">
                        <a class="page-link" href="?page=<?= min($totalPages, $page + 1) ?>&sort=<?= $sortKey ?>&dir=<?= $dir === 'ASC' ? 'asc' : 'desc' ?>">›</a>
                    </li>
                    <li class="page-item <?= ($page >= $totalPages) ? 'disabled' : '' ?>">
                        <a class="page-link" href="?page=<?= $totalPages ?>&sort=<?= $sortKey ?>&dir=<?= $dir === 'ASC' ? 'asc' : 'desc' ?>">&raquo;</a>
                    </li>
                </ul>
            </nav>
<?php endif; ?>
