  ...
  */
  ...
  public function getTariffModel(){
    ...
    public static function getModelIdForInvN($inv_n) {
         ...
         else return $result->fetch_assoc()['model_id'];
     }
    ...
    public static function getTovarFromArray($it_ch) {
        ...
        if(isset($it_ch['rent_cat_name'])) $tov->cat_name=$it_ch['rent_cat_name'];
        if(isset($it_ch['dog_name'])) $tov->cat_dog_name=$it_ch['dog_name'];
        if(isset($it_ch['color'])) $tov->model_color=$it_ch['color'];
        if(isset($it_ch['model'])) $tov->model_name=$it_ch['model'];
        if(isset($it_ch['set'])) $tov->model_set=$it_ch['set'];
        if(isset($it_ch['collateral'])) $tov->collateral=$it_ch['collateral'];
  ...
  public static function getFreeTovarsForModelIdAndOffice($model_id, $officeId){
    ...
    $mysqli = Db::getInstance()->getConnection();
    if ($officeId=='all') $query = "SELECT * from tovar_rent_items WHERE model_id = '$model_id' AND (`status`='to_rent' OR `status`='t_bron')";
    else $query = "SELECT * from tovar_rent_items WHERE model_id = '$model_id' AND item_place='$officeId' AND (`status`='to_rent' OR `status`='t_bron')";
  ...
  public static function getRostSizeArrayByModelId($modelId){
      ...
      while ($row= $result->fetch_assoc()) {
        ...
        if (!in_array($new_array, $rez)) $rez[]=$new_array;
      }
      return $rez;
  }
...
//    echo $query;
  ...
  public function updateExchRate()
  {
    ...
    $date = new \DateTime();
      ...
      $cur = $this->buy_price_cur;
        if ($this->buy_price_cur=='TBYR') $cur = 'BYN';
  ...
  public static function createFromDbArray (array $row){
    ...
    //arch info
    if(isset($row['item_arch_id'])) $t->item_arch_id = $row['item_arch_id'];
    if(isset($row['arch_date'])) $t->arch_date = $row['arch_date'];
    if(isset($row['arch_time'])) $t->arch_time = $row['arch_time'];
    if(isset($row['arch_who_id'])) $t->arch_who_id = $row['arch_who_id'];
    if(isset($row['out_status'])) $t->out_status = $row['out_status'];
    if(isset($row['out_sell_tbyr'])) $t->out_sell_tbyr = $row['out_sell_tbyr'];
    if(isset($row['out_sell_kassa'])) $t->out_sell_kassa = $row['out_sell_kassa'];
    if(isset($row['out_sell_usd'])) $t->out_sell_usd = $row['out_sell_usd'];
    if(isset($row['out_info'])) $t->out_info = $row['out_info'];
