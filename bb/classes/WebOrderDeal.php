...
class WebOrderDeal
{
    ...
    /**
     * Карточка брони из поля info — одной строкой обычного текста.
     *
     * В info лежит разметка карточки (bb/rent_orders.php выводит её как HTML),
     * но есть страницы, которые печатают то же поле экранированным — там теги
     * вылезали прямо в текст. Блочные теги заменяем пробелом, иначе даты, суммы
     * и комментарий слипаются в одно слово.
     */
    public static function infoToPlainText($html): string
    {
        // Пробел вместо любого тега, а не только закрывающего: иначе «26.08» и «14 дн.»,
        // разделённые лишь началом span, слипаются в «26.0814 дн.»
