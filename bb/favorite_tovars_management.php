    ...
    switch ($action) {
        ...
        case 'save':
            ...
            if (key_exists('pic_url', $_FILES) && $_FILES['pic_url']['name'] != '') {
                ...
                $savedPath = \bb\Base::processAndSaveImageAsWebp($dir_short, $_FILES['pic_url']['tmp_name'], $file_name);
                if ($savedPath)
                    $ft->setPicUrl($savedPath);

                //delete old file if exists
                if ($ft->getId() > 0) {
                    $old_ft = \bb\classes\FavoriteTovars::getBylId($ft->getId());
                    if ($old_ft->getPicUrl() != '') {
                        \bb\Base::delFile($old_ft->getPicUrl());
                    }
                }
            }//end if
        ...
        case 'delete':
            ...
            if ($ft->getId() > 0 && $ft->getPicUrl() != '') {
                \bb\Base::delFile($ft->getPicUrl());
            }
    ...
    <tbody>
        <?php if ($action = 'new' && $model_id > 0): ?>
            <tr class="table-info">
                ...
                <td>
                    <input class="form-control" type="file" name="pic_url" accept=".gif,.jpg,.jpeg,.png,.svg"
                        form="new_form">
                    <div class="form-floating">
                        <input class="form-control form-control-sm" name="pic_alt" form="new_form" id="pic_alt_new"
        ...
        <?php if ($fts && count($fts) > 0): ?>
            <?php foreach ($fts as $ft): ?>
                <tr <?= ($active_model_id == $ft->getModelId() ? 'class="table-warning"' : '') ?>>
                    <td>
                        <input type="hidden" name="model_id" value="<?= $ft->getModelId() ?>"
                            form="new_form_<?= $ft->getId() ?>">
                        <strong><?= $ft->getModelId() ?>:
                        </strong><?= \bb\classes\Model::getById($ft->getModelId())->getShortName(); ?>
                    </td>
                    <td>
                        <img src="<?= $ft->getPicUrl() ?>" width="70px">
                        <input class="form-control edit-field" type="file" name="pic_url" accept=".gif,.jpg,.jpeg,.png,.svg"
                            form="new_form_<?= $ft->getId() ?>">
                        <div class="form-floating">
                            <input class="form-control edit-field" data-controll="noquotes" name="pic_alt" id="pic_alt_new"
                                placeholder="alt для картинки" form="new_form_<?= $ft->getId() ?>"
                                value="<?= $ft->getPicAlt() ?>">
                    ...
                    <td>
                        <?= $ft->getNameText() ?>
                        <input type="text" class="form-control edit-field" name="name_text" value="<?= $ft->getNameText() ?>"
                            form="new_form_<?= $ft->getId() ?>">
                    </td>
                    <td>
                        <?= $ft->getDescription() ?>
                        <input type="text" class="form-control edit-field" name="description"
                            value="<?= $ft->getDescription() ?>" form="new_form_<?= $ft->getId() ?>">
                    </td>
                    <td>
