...
if ($srch!='') {
	...
	$query_mod = "SELECT * FROM tovar_rent WHERE model LIKE '%$srch%' ORDER by `model`";
	...
	if ($mod_num>0) {
		...
		$in_in=$mod['tovar_rent_id'];
	
		while ($mod=mysql_fetch_array($result_mod)) {
			$in_in=$in_in.','.$mod['tovar_rent_id'];
		}

		$srch_rules="WHERE model_id IN ($in_in) ";
		
		
	}
	else {$srch_rules='WHERE cat_id=\'def\' ';}
...
elseif ($cat_id=='def') {
	...
	$query_items = "SELECT * FROM tovar_rent_items ".$srch_rules."ORDER BY `model_id` ";
...
<div class="top_menu">
	...
	'.($_SESSION['level']!=3 ? '<a class="div_item" href="/bb/tovar.php">Новый товар</a>' : '').'	
</div>
		
<form name="cat_chose" action="tovar_rent_all.php" method="post" id="cat_ch_sel" style="display:inline-block;">
...
<form name="cat_chose" action="tovar_rent_all.php" method="post" style="display:inline-block;">
	<input type="hidden" name="cat_id" value="показать все товары">
</form>

<form name="srch" action="tovar_rent_all.php" method="post" style="display:inline-block;">
	<input type="text" name="srch" size="20" /> <input type="submit" name="action" value="найти модель" />
</form>					
...
<tr>
	...
	<td class="rent_all_t_mod">'.$model['model'].' ('.$items['model_id'].')</td>
	<td class="rent_all_t_col">'.$model['color'].'/'.$items['item_color'].'</td>
