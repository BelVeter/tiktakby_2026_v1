	...
	case 'сохранить':
        ...
        $mysqli = \bb\Db::getInstance()->getConnection();
			...
			if ($mod_num>0) {
				...
				$query_new_model = "INSERT INTO tovar_rent VALUES('', '$cat_id', '$producer_name', '$model_name', '$m_set_new', '$color_new', '$m_price_new', '$m_price_cur_new', '$lom_srok_new', '".time()."', '".$_SESSION['user_fio']."', '$model_addr_new', '$ph_addr_new', '$age_from', '$age_to', '$weight_from', '$weight_to', '$ny', '$zv', '$tale', '$rez1', '$rez2', '$collateral', '$m_sex')";
		...
		case 'обновить':
			...
			//получаем id категории (если нужно - создаем ее)
			if ($cat_select_new>0) {$cat_id=$cat_select_new;} //выбрана конкретная категория без изменений категории
			else {
...
//				if (!$result_upd2) {die('Сбой при доступе к базе данных: '.$query_upd2.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);}
				...
				if (!$result_items_upd) {die('Сбой при доступе к базе данных: '.$query_items_upd.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);}
