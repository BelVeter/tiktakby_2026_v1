...
if ($action_type=='arch_hist')	{// обязательно здесь, т.к. заканчивается дайем и потом ничего выводить не нужно
	...
	while ($dl_def=$result_dl_def->fetch_assoc()) {
		...
		if (!$result_cat) die('Сбой при доступе к базе данных: '.$result_cat.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);
		...
		$tov->model_color=='0' ? ($color='') : ($color=', цвет: '.$tov->model_color.': '.$tov->item_color); // если цвет отсутствует - то ничего не выводим, иначе выводим цвет
		  ...
		  $item_output.='
			<tr>
				<td><strong>№'.inv_print($dl_def['item_inv_n']).'</strong> '.addslashes($tov->cat_dog_name).' '.addslashes($tov->producer).', модель: '.addslashes($tov->model_name).$color.'</td>
...
if (mb_substr($item_inv_n, 0, 1, 'UTF-8')=='д' || mb_substr($item_inv_n, 0, 1, 'UTF-8')=='Д') {
	...
	if (!$result_d_num) die('Сбой при доступе к базе данных: '.$query_d_num.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);
...
$item_rows = $result->num_rows;
	...
	if ($item_rows==1) {
		...
		$item=$result->fetch_assoc();
			...
			if (!$result_tarif) die('Сбой при доступе к базе данных: '.$query_tarif.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);
	...
	if (($item['status']=='to_rent' || $item['status']=='bron' || ($item['status']=='t_bron' && $item['br_time']<time()))) {
		...
		$item_output.='<span style="color:red; font-size:18px;">'.$br_text.'</span>
				<table border="1" cellspacing="0">
				  <tr>
				    <td>Товар:</td>
					<td>'.addslashes($cat['dog_name']).' '.addslashes($model['producer']).', модель: '.addslashes($model['model']).$color.'</td>
				  </tr>
...
Дополнительная информация по сделке:<br/> <textarea cols="100" rows="3" name="deal_info" id="deal_info"></textarea><br />
    ...
    elseif ($item['status']=='rented_out' || $item['status']=='to_deliver') {
				...
				if (!$result_sub_cur) die('Сбой при доступе к базе данных: '.$query_sub_cur.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);
				...
				if (!$result_cl_def) die('Сбой при доступе к базе данных: '.$result_cl_def.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);
				...
				<table border="1" cellspacing="0">
				  <tr>
				    <td>Товар:</td>
					<td>'.addslashes($cat['dog_name']).' '.addslashes($model['producer']).', модель: '.addslashes($model['model']).addslashes($color).'</td>
				  </tr>
	...
	<tr>
		...
		<td>';
			//ищем место первой сдачи
				...
				if (!$result_first) die('Сбой при доступе к базе данных: '.$query_sub_first.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);
      ...
      if ($subDeals && count($subDeals)==1) $cur_date_change = true;
      else $cur_date_change = false;
...
switch ($sub_dl_def['type']) {
	case 'first_rent':
		...
		if (!$result_sub_pay) die('Сбой при доступе к базе данных: '.$query_sub_pay.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);
...
if (!$result_f) die('Сбой при доступе к базе данных: '.$query_f.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);
...
<strong>Последний использованный тариф:</strong><br />
		...
		<td><input type="button" name="button" id="button" value="Выбрать" onclick="apply_tarif(\\\'old\\\'); return false;" disabled />
		</td></tr>
		</table>
...
<select name="step" id="step">
	...
	<!--<option selected="selected" value="week">в неделю</option>-->
	<!--<option value="month">в месяц</option>-->
</select>
количество (д/н/м - по тарифу):<input type="number" name="rent_tenor" id="rent_tenor" onchange="daysChange();" value="" size="10"/><br/>

		<input type="button" value="пересчитать" id="calc_button" onclick="calculate(); return false;" /><br />
...
';
	...
	$query_cl_def = "SELECT * FROM clients WHERE client_id='".$dl_def['client_id']."'";
	$result_cl_def = $mysqli->query($query_cl_def);
	if (!$result_cl_def) die('Сбой при доступе к базе данных: '.$query_cl_def.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);
	$cl_def=$result_cl_def->fetch_assoc();
...
function pay_calc($deal_id, $ret_date) {
	...
	$query_dl_def = "SELECT * FROM rent_deals_act WHERE deal_id='$deal_id'";
	$result_dl_def = $mysqli->query($query_dl_def);
	if (!$result_dl_def) die('Сбой при доступе к базе данных: '.$result_dl_def.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);
	$dl_def=$result_dl_def->fetch_assoc();
