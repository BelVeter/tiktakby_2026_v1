  ...
  if (isset($_POST['action_command'])) {
    ...
    switch ($action) {
      case 'save':
        ...
        if (!$mwOld)
          $mwOld = \bb\classes\ModelWeb::getByModelId($modelId, 'ru');
        if (!$mwOld)
          $mwOld = new \bb\classes\ModelWeb($modelId, $lang);
        ...
        if (isset($_FILES['m_pic_big']) && $_FILES['m_pic_big']['name'] != '') {
          if ($_POST['m_pic_new_name'] == '')
            $_POST['m_pic_new_name'] = $_FILES['m_pic_big']['name'];
        ...
        if (isset($_FILES['l2_pic']) && $_FILES['l2_pic']['name'] != '') {
          if ($_POST['l2_pic_new_name'] == '')
            $_POST['l2_pic_new_name'] = $_FILES['l2_pic']['name'];
        ...
        if (isset($_FILES['logo_pic']) && $_FILES['logo_pic']['name'] != '') {
          if ($_POST['logo_pic_new_name'] == '')
            $_POST['logo_pic_new_name'] = $_FILES['logo_pic']['name'];
        ...
        if (isset($_FILES['dop_pic']) && $_FILES['dop_pic']['name'] != '') {
          if ($_POST['dop_pic_new_name'] == '')
            $_POST['dop_pic_new_name'] = $_FILES['dop_pic']['name'];
  ...
  <nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
      <div class="navbar-nav">
        ...
        <form>
          <a class="nav-item nav-link" href="#" onclick="document.querySelector('#tovar_cat').submit();">Перейти в
            категорию</a>
        </form>
      </div>
    </div>
  ...
  <div class="container-fluid mt-3">
    <div class="row">
      <div class="col d-flex justify-content-between">
        <button class="btn btn-outline-success" form="main-form" type="submit" name="action_command"
        ...
        <select class="form-select filter-select" data-live-search="true" style="width: unset" name="lang"
    ...
    <div class="row">
      <div class="col-7">
        <div class="row mt-2">
          ...
          <div class="col">
            <input class="form-control" name="name_main" id="name_main" type="text"
              value="<?= $mw->getItemNameMain(1) ?>">
          </div>
        </div>
        <div class="row mt-2">
          <div class="col-auto">
            <label class="form-label" for="l2_name" title="Название на карточке товара на страницах-листингах">Название
              (листинг)</label>
          </div>
          <div class="col">
        ...
        <div class="row mt-2">
          <div class="col-auto">
            <label class="form-label" for="title"
              title="Не должно содержать кавычки. Отражается в названии закладки браузера.">Title (для страницы)
              <sup>i</sup></label>
          </div>
          <div class="col">
        ...
        <div class="row mt-2">
          <div class="col-auto">
            <label class="form-label" for="meta_description"
              title="Не должно содержать кавычки. Отражается в результатах поиска гугл\яндекс.">Meta description
              <sup>i</sup></label>
          </div>
          <div class="col">
            <input class="form-control" name="meta_description" id="meta_description" type="text"
              value="<?= $mw->getMetaDescription() ?>">
          </div>
        </div>
        <div class="row mt-2">
          <div class="col-auto">
          ...
          <div class="col">
            <input class="form-control" name="breadcrumbs_name" id="breadcrumbs_name" type="text"
              value="<?= $mw->getBreadcrumbsName(1) ?>">
          </div>
        </div>
        <div class="row mt-2">
          <div class="col-auto">
            <label class="form-label" for="url_code" title="должен быть уникальным">URL код страницы
              <sup>i</sup></label>
          </div>
          <div class="col">
        ...
        <div class="row mt-2">
          <div class="col-auto">
            <label class="form-label" for="m_a_title"
              title="Важно для SEO, не должен будлировать title страницы. Для человека. Всплывает, если держать курсор над ссылкой.">Title
              для ссылки на страницу <sup>i</sup></label>
          </div>
          <div class="col">
        ...
        <div class="row mt-2">
          <div class="col-auto">
            <label class="form-label" for="m_a_keywords" title="Ключевые слова для своего поиска по сайту">Ключевые
              слова для поисковика <sup>i</sup></label>
          </div>
          <div class="col">
      ...
      <div class="col-5">
        <div class="row">
          ...
          <div class="col">
            <input class="form-control d-inline" name="m_pic_alt" type="text" placeholder="alt для картинки"
              value="<?= $mw->getMPicAlt() ?>">
          </div>
        </div>
        <div class="row photo-row">
          ...
          <div class="col">
            <label class="form-label d-block" title="Обязательно квадратная. Разрешение от 1000х1000 пикселей.">Главная
              картинка на страницу товара<sup>i</sup></label>
            <span class="file-name d-none">Имя файла:</span><input class="form-control w-100 d-none new-filename"
        ...
        <div class="row photo-row">
          ...
          <div class="col">
            <label class="form-label d-block" title="Обязательно квадратная. Разрешение от 200x200 пикселей.">Картинка
              на листинг<sup>i</sup></label>
            <span class="file-name d-none">Имя файла:</span><input class="form-control w-100 d-none new-filename"
        ...
        <?php if (is_array($mw->getDopPictures()) && count($mw->getDopPictures()) > 0): ?>
          <?php foreach ($mw->getDopPictures() as $pic): ?>
            <div class="row photo-row">
              ...
              <div class="col-9"><input class="file-del-checkbox" type="checkbox" name="dop_to_del[]"
                  value="<?= $pic->getSrc() ?>"><span data-copy="1" style="font-size: 10px; cursor: pointer;"
                  title="Click to copy!"><?= $pic->getSrc() ?></span></div>
            </div>
          <?php endforeach; ?>
        <?php endif; ?>
        <div class="row photo-row">
          ...
          <div class="col">
            <label class="form-label d-block" title="Обязательно квадратная. Разрешение от 1000x1000 пикселей.">Доп.
              картинка<sup>i</sup></label>
            <span class="file-name d-none">Имя файла:</span><input class="form-control w-100 d-none new-filename"
        ...
        <div class="row photo-row">
          ...
          <div class="col">
            <input class="form-control" name="tarif_base_days" type="number" min="1"
              value="<?= $mw->getTarifBaseDays() ?>">
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="col" id="add-cat-container">
        <?php foreach ($mw->getAdditionalCategories() as $addCat): ?>
          <div class="form-check">
            <input class="form-check-input" type="checkbox" name="dop_cat[]" value="<?= $addCat[0] ?>"
              id="cat-<?= $addCat[0] ?>" checked>
            <label class="form-check-label" for="cat-<?= $addCat[0] ?>"><?= $addCat[1] ?></label>
            <input class="form-control" type="text" name="add_cat_pic_url_<?= $addCat[0] ?>" value="<?= $addCat[2] ?>"
              placeholder="url доп. картинки листинг" data-controll="url-file">
          </div>
        <?php endforeach; ?>
      </div>
    </div>
    <div class="row mt-3">
      <div class="col"><button class="btn btn-outline-success" form="main-form" type="submit" name="action_command"
          value="save">Сохранить</button></div>
    </div>
  </form>
