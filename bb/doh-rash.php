...
switch ($action) {
	...
	case 'сохранить':  //оплата
		...
		switch ($channel) {
			...
			if ($type1=='shift') {
				...
				if (!$result_ins) {die('Сбой при доступе к базе данных: '.$ins_q.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);}
				...
				if (!$result_upd) {die('Сбой при доступе к базе данных: '.$upd_q.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);}
	...
	case 'удалить':  //оплата
      ...
      $dohRash = \bb\classes\DohRash::getById($dr_id);
        if ($dohRash) $dohRash->logBeforeDelete();
  ...
  case 'update_rash':  //оплата
    ...
    if (!$result_upd) {die('Сбой при доступе к базе данных: '.$query.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);}
  break;
...
if (!$result_rd) {die('Сбой при доступе к базе данных: '.$rd_q.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);}
...
if (!$result_rl) {die('Сбой при доступе к базе данных: '.$rl_q.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);}
...
if (!$result_zp) {die('Сбой при доступе к базе данных: '.$zp_q.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);}
...
if (!$result_dr) {die('Сбой при доступе к базе данных: '.$dr_q.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);}
/*
