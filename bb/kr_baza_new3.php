...
elseif ($model_srch!='') {
    $srch_ar[]="model_id IN (SELECT tovar_rent_id FROM tovar_rent WHERE tovar_rent.model LIKE '%$model_srch%' OR tovar_rent.producer LIKE '%$model_srch%')";
}
...
<form name="cat_chose" method="post" id="cat_ch_sel" style="display:inline-block;">
					<select name="cat_id" class="form-control form-control-sm selectpicker form-control" style="width: 100px;" data-live-search="true" id="cat_select" onchange="$(\'#model_srch\').val(\'\'); $(\'#inv_srch\').val(\'\'); document.getElementById(\'cat_ch_sel\').submit(); return false;">
		...
		<tr>
			<th class="krb_cat">Категория<br />
			    <select form="cat_ch_sel" name="last_rent" id="last_rent" onchange="$(\'#inv_srch\').val(\'\'); document.getElementById(\'cat_ch_sel\').submit(); return false;" style="width:80px;">
						...
						<option value="elz" '.sel_d('3', $last_rent).'>Последний прокат</option>
				</select>
...
elseif ($item_def['status']=='bron') {
	...
	echo'
		<tr id="tr_'.$item_def['item_id'].'" '.$status_style.'>
			<td>'.$cat_name.'</td>
			<td style="overflow: hidden;">'.$producer_name.'<br />['.($model_web['sort_n'] ?? '').']</td>
			<td> <div class="t_pic">
