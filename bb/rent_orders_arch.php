...
//основной запрос
$query_or = "SELECT * FROM rent_orders_arch WHERE arch_time>".$from->getTimestamp()." ORDER BY arch_time DESC";
//echo $query_or;
...
echo '
<h1>Брони/заявки, удаленные в течение последних 7-и дней</h1>
...
while ($ord = $result_or->fetch_assoc()) {
	...
	echo '
		...
		<td '.((($br_line->type2=='bron' || $br_line->type2=='deliv') && $br_line->appr_id>0) ? 'style="background-color:#acf398;"' : '').'>'.$br_line->info.' <br />'.$br_line->info2.'<br>
