...
if (!$result_r) {
	die('Сбой при доступе к базе данных: ' . $query_r . ' (' . $mysqli->connect_errno . ') ' . $mysqli->connect_error);
}
...
if ($result_r->num_rows > 0) {
	...
	if (!$result_rem_id)
		die('Сбой при доступе к базе данных: ' . $query_r_id . ' (' . $mysqli->connect_errno . ') ' . $mysqli->connect_error);

	//assigning number of remont to array
	while ($rem_id = $result_rem_id->fetch_assoc()) {
		if (!array_key_exists($rem_id['appr_id'], $remont_users_count))
			continue; //if not active user - skip
		else
			$remont_users_count[$rem_id['appr_id']] += $rem_id['COUNT(`order_id`)'];
	}
...
if (isset($_POST['action'])) {
	...
	switch ($action) {
		...
		case 'tovar_sell':
			...
			if ($ofNum < 1)
				$ofNum = \bb\models\Office::getCurrentOffice()->getIdOffice();
		...
		case 'удалить':
			...
			if (!$result_tov) {
				die('Сбой при доступе к базе данных: ' . $q_tov . ' (' . $mysqli->connect_errno . ') ' . $mysqli->connect_error);
			}
			...
			if (!$result) {
				die('Сбой при блокировке таблиц MYSQL: ' . $query . ' (' . $mysqli->connect_errno . ') ' . $mysqli->connect_error);
			}
			break;
	}
}
...
if (!$result_or_new) {
	die('Сбой при доступе к базе данных: ' . $query_or_new . ' (' . $mysqli->connect_errno . ') ' . $mysqli->connect_error);
}
...
if (!$result_free) {
	die('Сбой при доступе к базе данных: ' . $query_free . ' (' . $mysqli->connect_errno . ') ' . $mysqli->connect_error);
}
...
if (!$result_of) {
	die('Сбой при доступе к базе данных: ' . $q_of . ' (' . $mysqli->connect_errno . ') ' . $mysqli->connect_error);
}
...
while ($ord = $result_or->fetch_assoc()) {
	...
	if ($br_line->type2 == 'remont') {
		...
		if ($br_line->item_to_move > 0) {//=isVPuti
			...
			foreach ($ofs as $of) {
				if ($of->number == $br_line->item_place)
					continue;
	...
	if ($br_line->isLastRent())
		echo '<div style="background-color: orangered">Последний прокат!</div>';
	//customer info
	echo '
