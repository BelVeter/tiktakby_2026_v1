...
### Задача 5: `tovar_del.php` — чистить все связи или отказывать

**Files:**
- Modify: `bb/tovar_del.php:254-292` (case `удалить все (модель и все товары)`)

**Обоснование:** сейчас удаляются только `tovar_rent` и `tovar_rent_items`. Это и породило 222 висячие заявки, 172 тарифа, 15 фото и 3 архивных юнита без модели.
...
**⚠️ РЕШЕНИЕ ВЛАДЕЛЬЦА ДО ЗАПУСКА: юнит 719242 сейчас в активной аренде** (сделка 137506). После переноса на модель 1069 продление этой аренды будет считаться по тарифам 1069, то есть **дороже**, чем клиент арендовал изначально. Варианты: (а) слить сейчас и принять это; (б) дождаться возврата юнита и слить после. Миграция написана так, что безопасна в обоих случаях, но выбор — бизнесовый.
...
### Задача 10: Вычистить висячие ссылки

**Files:**
- Create: `database/migrations/2026_07_27_140000_cleanup_dangling_model_refs.php`

**Что чистим и что нет:**

| Таблица | Строк | Решение |
|---|---|---|
| `rent_tarif_act` / `rent_tarif_prev` | 172 | удалить — тарифы несуществующей модели бесполезны |
| `dop_photos` | 15 | удалить — фото несуществующей модели не отрисуются |
| `rent_orders_arch` | 222 | **оставить**, но обнулить `model_id` → `0` нельзя: это ломает связь. Оставляем как есть, только логируем — это исторические заявки, их текст (`info`) читается оператором и без модели |
| `tovar_rent_items_arch` | 3 | **оставить** — архивные юниты участвуют в исторических отчётах по инвентарю |
