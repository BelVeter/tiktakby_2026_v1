...
### Задача 17: Нормализовать существующие названия производителей ✅ ВЫПОЛНЕНО (Правка 7)

**Files:**
- Create: `database/migrations/2026_07_28_170000_normalize_producer_spellings.php`
- Modify: `app/Http/Controllers/SearchController.php` (301 для переименованных)

> ⚠️ **Ловушка MySQL, из-за которой очевидное условие не работает.** Коллации `utf8mb3_general_ci` — PAD SPACE: при сравнении **замыкающие** пробелы игнорируются. Поэтому `WHERE producer <> TRIM(producer)` находит только названия с **ведущим** пробелом, а `'Chicco '` (77 моделей у `'Chicco'` против 1 у `'Chicco '`) считается равным `'Chicco'` и в выборку не попадает. Правильно — либо `WHERE producer <> BINARY TRIM(producer)`, либо безусловный `UPDATE` без `WHERE`. В миграции используется безусловный вариант.

**Та же коллация меняет и смысл задачи.** Раз `_ci` + PAD SPACE, то `WHERE producer='Chicco'` уже возвращает и `'Chicco '`, а `'THULE'` уже находит `'Thule'` — значит варианты по регистру и хвостовому пробелу на поиск и на логотипы **не влияли**, это косметика. Реально ломали привязку только два случая: **ведущий** пробел (` Simple Parenting` — коллация его не игнорирует) и разные слова (`I love mum, РФ`).

**Почему это чинит логотипы.** `Model::getModelIdsArrayByProducer()` матчит производителя точным сравнением, и на нём стоят `ModelWeb::loadLastProducerLogo()` и `updateLogoUrlForAll()`. После слияния бренд Doona виден целиком — 6 моделей вместо 3, — и рассылка логотипа из админки дотягивается до трёх страниц, где его не было. Ничего нового писать не пришлось: механизм уже был, его расщепляли данные.

**Спорные пары разобраны:**
- `I love mum` (5) + `I love mum, РФ` (11) → **`I love mum`**. «, РФ» — пометка о стране. У всех 16 моделей ноль живых единиц и ни одной веб-страницы, на сайте слияние не видно вообще.
- ` Simple Parenting` (3) + `Simple Parenting Doona` (3) → **`Simple Parenting Doona`**. Одна компания: Simple Parenting выпускает линейку Doona; в первой группе автокресла-люльки «Doona», «Doona X», «База Doona», во второй велосипеды «Liki Trike S1». Токен «Doona» уже стоит во всех title и слагах обеих групп, и у этой группы больше живых единиц (11 против 4).

**URL производителя редиректом из БД не чинится:** `CheckRedirects` матчит только путь, без query-строки, а лендинг `/ru/producer?producer=...` индексируемый (self-canonical). 301 отдаёт сам `producerFilter()` по карте `PRODUCER_RENAMED`.

**Итог:** 8 групп сведены к одному написанию, строк с краевыми пробелами 0, различных производителей 326. Осталось на решение владельца: мусор в поле производителя — `рб` (31 страница), `аксессуар` (19) / `акссесуар` (8, опечатка того же слова), `Китай`; и 107 L3-страниц без логотипа при наличии логотипа у бренда.
...
## Фаза 7. Справочник производителей ✅ ВЫПОЛНЕНО

Спека: `docs/superpowers/specs/2026-08-14-producers-directory-design.md`.
План: `docs/superpowers/plans/2026-08-15-producers-directory.md`.
Ветка: `feature/producers-directory`.

**Отклонение от первоначального наброска.** Здесь планировался `tovar_rent.producer_id` с внешним ключом. По решению владельца — нет: строку читают 52 файла, переписывать их ради чистоты схемы несоразмерно риску. `tovar_rent.producer` остаётся свободным varchar, таблица `producers` — источник для записи и для витрин (главная, L3), не единственный источник правды со связью по ключу.

**Что сделано:**
- Таблица `producers` (`name` UNIQUE, `name_norm`, `logo`, `comment`, `is_active`) + миграция засева из всех существующих значений `producer` (объединение `tovar_rent`/`tovar_rent_items`/`tovar_rent_items_arch`, чтобы не терять archived-only написания).
- Детектор дублей расширен вторым сигналом: триграммный Jaccard (уже был) ловит опечатки в длинных названиях, добавленное расстояние Левенштейна — в коротких (`аксессуар`/`акссесуар`, `Chicco`/`Chico`, `Батик`/`Ботик`). Оба сигнала — в `Similarity`, не смешаны: у категорий поведение не изменилось.
- Живой поиск + попапы создания/редактирования на `tovar_new_mod.php` (справочник, только активные, приоритет брендов текущей категории), отдельный живой поиск на `tovar_new.php` (не справочник — существующие значения `producer` в выбранной категории, каскад `cat_ch()`/`prod_ch()`/`model_ch()` не тронут, только довешен сброс при смене категории).
- Логотип переведён с рассылки копий по `rent_model_web` всех моделей бренда на одну строку в `producers.logo`, с фолбэком на старую колонку. Заодно нашёлся и починен побочный эффект: наивный `JOIN producers` в `getAllProducersTovExists()` добавлял на главную бренды вроде «Китай» (логотип и живые единицы на разных моделях) — восстановлена старая связка «единица и логотип на одной строке» для отбора, при этом URL логотипа берётся уже из справочника.
- Переименование — с предпросмотром масштаба, одной транзакцией на 4 таблицы, запретом переименования в уже существующее имя (это слияние, делается миграцией через PR, не из попапа).

**Не сделано (сознательно, см. спеку):** SEO-лендинг производителя и таблица псевдонимов для 301 при переименовании; страница управления брендами; разбор карнавальной свалки в `producer` по существу (гасится через `is_active` по уже названному в спеке списку). Слияние оставшихся дублей (`Kinderkraft`/`Kinder Kraft`, `Baby Mamy`/`Babymamy`, `Maxi-Cosi`/`Maxi Cosi`, `аксессуар`/`акссесуар`) — отдельная миграция на прод-данных, список утверждает владелец перед деплоем (`docs/prod_pending.md`).
