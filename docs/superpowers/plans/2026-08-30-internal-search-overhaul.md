...
### Task 13: Прод-хвосты, найденные по дороге (последним пунктом)

Не относится к поиску, но обнаружено при разборе и не должно потеряться.

**13.1. Ночной крон остатков касс — разобраться, нужен ли он вообще.**

Факты (проверено 2026-08-30/31):
- Крон-строка `0 0 * * * /usr/local/bin/php .../bb/auto_ostatki.php` **не выполняется
  с 22.06.2026** — на новом сервере `/usr/local/bin/php` не существует (в
  `/usr/local/bin/` только симлинк `lsphp` и `wp`). Это наследство переезда с cPanel:
  блок в crontab помечен `CPANEL2ISPMGR_CRON_BEGIN`.
- Ошибку не видно, потому что в crontab стоит `MAILTO=""`.
- Доказательство простоя: `bb/log_auto.txt` последний раз дописан 22.06.2026 13:36.
- Сам скрипт **сейчас ничего не делает**: в `bb/auto_ostatki.php` строка
  `//\bb\KassaOstatok::calculateAndSaveAllUpToday();` закомментирована, остался
  только `file_put_contents` с меткой времени.
- **Данные при этом в порядке**: таблица `kassas` наполняется штатно — 126 строк за
  август по 30 дням, последняя запись 30.08 15:00, апрель–август без провалов,
  все строки `k_type='final'`. Значит остатки пишет другой путь (вероятно закрытие
  кассы в админке), а не этот крон.

- [ ] Найти, что именно пишет `kassas` сегодня (искать вызовы `KassaOstatok` в `bb/`).
- [ ] Решить: крон восстанавливать (тогда починить путь на `/opt/alt/php74/usr/bin/php`
      и раскомментировать вызов) или удалить строку из crontab как мёртвую.
- [ ] Если восстанавливать — сверить, не будет ли двойной записи с тем путём,
      который наполняет таблицу сейчас.

**13.2. Мусор в `laravel.log` — устранить источник.**

- [ ] `resources/views/not_found.blade.php:10` пишет `Log::info("View loaded: " . $currentPath)`
      на каждый 404. За полгода это дало 57 МБ в `storage/logs/laravel.log` без ротации.
      Лог заархивирован и обнулён 31.08.2026 (`laravel-2026-08-31.log.gz`, 2.8 МБ),
      но источник остался и наполнит его снова.
      Решить: убрать строку совсем либо увести в отдельный `daily`-канал с retention.
- [ ] Починить сами 404, которые её кормят (~350 строк в день):
      `/fonts/Gilroy-Light.otf` (188/день), `/public/favicon-32x32.png` (156/день),
      `/fonts/Gilroy-ExtraBold.otf` (45/день), `/public/css/bootstrap.min.css.map` (21/день).
      Шрифты в CSS прописаны как `/public/fonts/...`, а запрашиваются с `/fonts/...`.
- [ ] Отдельно от этого: строка про `bb_nav_badge.php` дала 127 тыс. записей в логе,
      но прекратилась 30.06.2026 — уже починена, трогать не нужно.

**13.3. Демон для Meilisearch — ждём hoster.by.**

- [ ] 30.08.2026 отправлено письмо в `support@hoster.by` с `info@tiktak.by`
      (копия в «Отправленных»): разрешён ли постоянный фоновый процесс, точные лимиты
      LVE (NPROC/PMEM/EP/CPU), не отправит ли Imunify360 бинарник в карантин,
      чем поднимать процесс без systemd. **До ответа тему не двигаем.**
- [ ] Если ответ «нельзя» — рассмотреть TNTSearch (чистый PHP, индекс файлом,
      никаких процессов) как способ получить саджест и устойчивость к опечаткам.
...
| Прод-хвосты (крон касс, мусор в логе, демон) | Task 13 |
