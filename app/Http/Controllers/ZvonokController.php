...
class ZvonokController extends Controller
{
    ...
    public function addCall(Request $req){
        Zvonok::addLitZvonok($req->input('fio'), $req->input('phone'), $req->input('info'));
      ...
      if ($action = $mysqli->real_escape_string($req->input('action'))) {
        ...
        switch ($action){
          case 'get-offices-for-model':
            ...
            if (!$offsHasItemArray) $offsHasItemArray = [];
        ...
        switch ($action) {
          ...
          case 'new_bron'://first step (no brons saved, just get info)
            ...
            foreach ($freePeriods as $fp){
              ...
              if (!$exists) $freePeriodsToShow[($fp->to_kb->getTimestamp()-$fp->from_kb->getTimestamp())] = $fp;
            }
          ...
          if (is_array($freePeriods) && count($freePeriods)>0) {
            ...
            if (!$hasFrees){//if no free preiods
              ...
              if ($eventDate->format('d.m.Y') != $today->format('d.m.Y')) $f->addFromDay($prevDay);
...
//            ///!!!
          ...
          case 'zayavka':
            ...
            if (!$result_item_def) {die('Сбой при доступе к базе данных: '.$query.' ('.$mysqli->connect_errno.') '. $mysqli->connect_error);}
